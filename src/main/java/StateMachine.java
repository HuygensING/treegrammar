...
public class StateMachine {
  ...
  public void processInput(String tag) {
    ...
    for (TransitionRule rule : rules) {
      // dit klopt niet!
      if (rule.lefthandsideIsApplicableFor(((TagNode) pointerToCurrentNode).tag)) {
        found = true;
        theRule = rule;
        break;
      }
    ...
    if (!found) {
      throw new RuntimeException("No transition rule found! Current state: " + this.pointerToCurrentNode.toString() + " -> " + tag);
    }
    ...
    if (((TagNode) theRule.righthandside.root).tag.equals(tag)) {
